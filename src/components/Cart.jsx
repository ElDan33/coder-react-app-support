    ...
    return (
        <>
            <div className={darkMode ? "App-header-dark w-full lg:w-screen h-full lg:h-screen": "App-header w-full lg:w-screen h-full lg:h-screen"}>
                ...
                {/* {wannaClear ? <CustomModal  wannaClear={wannaClear} setWannaClear={setWannaClear} cartClear={cartClear}/> : null}
                {wannaRemove ? <CustomModal wannaRemove={wannaRemove} /> : null} */}
                                    ...
                                    cart.map((item, index)=>{
                                        ...
                                        return(
                                            ...
                                            >   
                                                ...
                                                <span className="basis-1/6 mt-12 ">$ {item?.item?.price * presentationMultiplier}</span>
                                                <span className="basis-1/6 mt-12 ">{item?.item?.count}</span>
                                                <span className="basis-1/6 mt-12 ">$ {subtotal}</span>
                                                <button 
                                                        className="mb-4 pr-2" 
                                                        aria-label="Delete product"
                                                        title="Delete product" 
                                                        onClick={()=>removeFromCart(item?.item?.id)}
                                                        // onClick={() => (removeHandler() && <CustomModal itemId={item?.item?.id}/>)}
                                                    >
