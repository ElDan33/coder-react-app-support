...
const ItemCount = ({count, stockToNumber, countIncrease, countDecrease, addCartHandler, countStock}) => {
    ...
    return (
            <div className="flex-col text-center w-full border border-2 rounded-b-3xl bg-amber-50 p-4">
                <div className={darkMode ? "flex justify-between bg-gray-600 mb-2  border-2 border-solid border-gray-300 rounded-lg border-b-gray-400 border-r-gray-400 bg-white" : "flex justify-between bg-white mb-2  border-2 border-solid border-gray-300 rounded-lg border-b-gray-400 border-r-gray-400 bg-white"}>
                    <button onClick={countDecrease} className="font-bold text-3xl p-4 text-white text-shadow-h1 w-20 bg-green-300 hover:bg-green-400 active:bg-orange-200 active:border-orange-400  rounded-lg">-</button>
                    <div className="text-white text-shadow-h1 text-3xl p-4">
                        {count}
                    </div>
                    <button onClick={countIncrease} className="font-bold text-3xl p-4 text-white text-shadow-h1 w-20 bg-green-300 hover:bg-green-400 active:bg-orange-200 active:border-orange-400  rounded-lg">+</button>
                </div>
                {(stockToNumber > 0 
